...
public:
    ...
    void exchangeNodes(int index1, int index2);
...
template<typename T>
List<T>::List()
    :firstPtr{nullptr},lastPtr{nullptr},listLength{-1}
{
}
...
template<typename T>
void List<T>::exchangeNodes(int index1, int index2)
{
    auto currentNode1 = firstPtr;
    auto currentNode2 = firstPtr;
    std::shared_ptr<ListNode<T>> behindNode1;
    std::shared_ptr<ListNode<T>> behindNode2;
    int tempCount1 = 0;
    int tempCount2 = 0;
    if(index1 > this->listLength || index2 > this->listLength)
        throw std::out_of_range("index out of range");

    while (currentNode1 != nullptr && currentNode2 != nullptr){
        if(tempCount1 < index1){
            behindNode1 = currentNode1;
            currentNode1 = currentNode1->next;
            tempCount1++;
        }
        if(tempCount2 < index2){
            behindNode2 = currentNode2;
            currentNode2 = currentNode2->next;
            tempCount2++;
        }
        if(tempCount1 == index1 && tempCount2 == index2)
            break;
    }
    std::swap(behindNode1->next,behindNode2->next);
    std::swap(currentNode1->next,currentNode2->next);
}
